...
.collapser.expanded + * {height: auto;}';
	...
	function testNamespaces() {
		...
		$sExpected = '@namespace toto "http:
@namespace "http:
@namespace foo url("http:
@namespace foo url("http:
foo|test {gaga: 1;}
|test {gaga: 2;}';
		$this->assertSame($sExpected, $oDoc->render());
...
@media screen {#foo.bar {position: absolute;}}';
	...
	function testUrlInFile() {
		...
		$sExpected = 'body {background: #fff url("http:
body {background-url: url("http:
		$this->assertSame($sExpected, $oDoc->render());
	}
...
body {background-url: url("http:
	...
	function testFlatCommentExtracting() {
		$parser = new Parser('div {left:10px; text-align:left;}');
		$doc = $parser->parse();
